...
export class MealService {
  ...
  private computeTotals(items: FoodItemPublic[], recipes: RecipeInMeal[]): MealTotals {
    const sum = (arr: { calories: number; protein: number; carbs: number; fat: number }[]) =>
      arr.reduce((acc, i) => ({
  ...
  private mealTypeToName(mealType?: string): string {
    const map: Record<string, string> = {
      ...
      snack: 'Lanche', dinner: 'Jantar', supper: 'Ceia',
    };
    return map[mealType ?? ''] ?? 'Refeição';
  }

  private toMealPublic(meal: { items: FoodItemPublic[]; recipes: { id: string; name: string; items: FoodItemPublic[] }[]; [key: string]: unknown }): MealPublic {
    const { items, recipes, ...rest } = meal;
    const enrichedRecipes: RecipeInMeal[] = recipes.map((recipe) => {
      const totals = recipe.items.reduce((r, i) => ({
        calories: r.calories + i.calories, protein: r.protein + i.protein,
        carbs: r.carbs + i.carbs, fat: r.fat + i.fat,
      }), { calories: 0, protein: 0, carbs: 0, fat: 0 });
      return { id: recipe.id, name: recipe.name, ...totals, items: recipe.items };
    });
    return { ...rest, items, recipes: enrichedRecipes, totals: this.computeTotals(items, enrichedRecipes) } as MealPublic;
  }

  async create(callerId: string, patientId: string, dto: CreateMealDto): Promise<MealPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const meal = await this.prisma.meal.create({
        data: { name: this.mealTypeToName(dto.mealType), kind: dto.kind, mealType: dto.mealType, time: dto.time, date: dto.date ? new Date(dto.date) : undefined, patientId },
        select: mealSelect,
      });
  ...
  async findAllByPatient(callerId: string, patientId: string, kind?: MealKind): Promise<MealPublic[]> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const meals = await this.prisma.meal.findMany({ where: { patientId, ...(kind && { kind }) }, select: mealSelect, orderBy: { date: 'desc' } });
  ...
  async findOne(callerId: string, patientId: string, id: string): Promise<MealPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const meal = await this.prisma.meal.findFirst({ where: { id, patientId }, select: mealSelect });
  ...
  async update(callerId: string, patientId: string, id: string, dto: UpdateMealDto): Promise<MealPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const meal = await this.prisma.meal.update({
        where: { id, patientId },
        data: { ...(dto.name !== undefined && { name: dto.name }), ...(dto.mealType !== undefined && { mealType: dto.mealType }), ...(dto.time !== undefined && { time: dto.time }), ...(dto.date !== undefined && { date: new Date(dto.date) }) },
        select: mealSelect,
      });
      return this.toMealPublic(meal);
    } catch (error) {
      mapPrismaError(error, 'Erro ao atualizar refeição', { p2025: 'Refeição não encontrada' });
    }
  }

  async remove(callerId: string, patientId: string, id: string): Promise<{ id: string }> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      return await this.prisma.meal.delete({ where: { id, patientId }, select: { id: true } });
    } catch (error) {
      mapPrismaError(error, 'Erro ao deletar refeição', { p2025: 'Refeição não encontrada' });
    }
  }

  async addFoodItem(callerId: string, patientId: string, mealId: string, dto: AddFoodItemDto): Promise<FoodItemPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
  ...
  async addPrivateFoodItem(callerId: string, patientId: string, mealId: string, dto: AddMealPrivateFoodItemDto): Promise<FoodItemPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const privateFood = await this.prisma.privateFood.findFirst({ where: { id: dto.privateFoodId, patientId }, select: { id: true, calories: true, protein: true, carbs: true, fat: true, servingSize: true } });
  ...
  async removeItem(callerId: string, patientId: string, mealId: string, itemId: string): Promise<{ id: string }> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const { count } = await this.prisma.foodItem.deleteMany({ where: { id: itemId, meal: { id: mealId, patientId } } });
  ...
  async addRecipe(callerId: string, patientId: string, mealId: string, dto: AddMealRecipeDto): Promise<MealPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const recipe = await this.prisma.recipe.findFirst({ where: { id: dto.recipeId, patientId }, select: { id: true } });
      if (!recipe) throw new NotFoundException('Receita não encontrada');
      const meal = await this.prisma.meal.update({ where: { id: mealId, patientId }, data: { recipes: { connect: { id: recipe.id } } }, select: mealSelect });
      return this.toMealPublic(meal);
    } catch (error) {
      mapPrismaError(error, 'Erro ao adicionar receita à refeição', { p2025: 'Refeição não encontrada' });
    }
  }

  async removeRecipe(callerId: string, patientId: string, mealId: string, recipeId: string): Promise<MealPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const meal = await this.prisma.meal.update({ where: { id: mealId, patientId }, data: { recipes: { disconnect: { id: recipeId } } }, select: mealSelect });
      return this.toMealPublic(meal);
    } catch (error) {
      mapPrismaError(error, 'Erro ao remover receita da refeição', { p2025: 'Refeição não encontrada' });
    }
  }
}
