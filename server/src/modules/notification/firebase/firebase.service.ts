...
export class FirebaseService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    const appUrl = this.configService
      ...
      ?.split(',')[0]
      ?.trim();
    this.clickLink = appUrl?.startsWith('https')
      ? `${appUrl}/app/home`
      : undefined;
  }
    ...
    try {
      ...
      if (result.failureCount > 0) {
        const reasons = result.responses
          ...
          .join(', ');
        this.logger.warn(
          `Notificações: ${result.successCount} enviadas, ${result.failureCount} falharam (${reasons})`,
        );
      } else {
      ...
      this.logger.error(
        `Notificações não enviadas: ${(error as Error).message}`,
      );
