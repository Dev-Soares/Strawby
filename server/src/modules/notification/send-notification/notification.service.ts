...
export class NotificationService {
  ...
  async sendOne(userId: string, title: string, body: string) {
    const tokens =
      await this.notificationTokenService.getTokensByUserId(userId);

    if (tokens.length === 0) return;

    try {
      const invalidTokens = await this.firebaseService.sendMulticast(tokens, {
        title,
        body,
      });
  ...
  async sendMany(userIds: string[], title: string, body: string) {
    const allTokens = await Promise.all(
      userIds.map((userId) =>
        this.notificationTokenService.getTokensByUserId(userId),
      ),
    );
