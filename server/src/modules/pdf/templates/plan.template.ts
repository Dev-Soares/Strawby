...
const ICON_PATHS: Record<string, string> = {
  ...
  cookie:    'M167.31,160.69a16,16,0,1,1-22.62,0A16,16,0,0,1,167.31,160.69Zm-86.62-8a16,16,0,1,0,22.62,0A16,16,0,0,0,80.69,152.69Zm14.62-33.38a16,16,0,1,0-22.62,0A16,16,0,0,0,95.31,119.31Zm48-6.62a16,16,0,1,0,0,22.62A16,16,0,0,0,143.31,112.69ZM236,128A108,108,0,1,1,128,20a12,12,0,0,1,12,12,36,36,0,0,0,36,36,12,12,0,0,1,12,12,36,36,0,0,0,36,36A12,12,0,0,1,236,128Zm-24.67,10.65A60.17,60.17,0,0,1,165,91a60.17,60.17,0,0,1-47.66-46.32,84,84,0,1,0,94,94Z',
  bowlFood:  'M224,100h-4.78a92,92,0,0,0-182.44,0H32a12,12,0,0,0-12,12,108.38,108.38,0,0,0,56,94.68V208a20,20,0,0,0,20,20h64a20,20,0,0,0,20-20v-1.32A108.38,108.38,0,0,0,236,112,12,12,0,0,0,224,100ZM170.29,60.06A92,92,0,0,0,127.19,100H106a68.27,68.27,0,0,1,62-40C168.76,60,169.52,60,170.29,60.06Zm17.22,19.08A67.66,67.66,0,0,1,194.92,100H156.13A67.91,67.91,0,0,1,187.51,79.14ZM128,44c.83,0,1.65,0,2.48.06A92.3,92.3,0,0,0,80.37,100H61.08A68.1,68.1,0,0,1,128,44Zm35,144.39a12,12,0,0,0-7,10.91V204H100v-4.7a12,12,0,0,0-7-10.91A84.32,84.32,0,0,1,44.87,124H211.13A84.32,84.32,0,0,1,163,188.39Z',
};

function iconSvg(name: keyof typeof ICON_PATHS, size: number, color: string): string {
  const d = ICON_PATHS[name];
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 256 256" fill="${color}"><path d="${d}"/></svg>`;
}

const MEAL_CONFIG: Record<string, { label: string; accent: string; light: string; accentText: string; icon: keyof typeof ICON_PATHS }> = {
  breakfast: { label: 'Café da manhã', accent: '#ea580c', light: '#fed7aa', accentText: '#c2410c', icon: 'coffee' },
  lunch:     { label: 'Almoço',        accent: '#16a34a', light: '#bbf7d0', accentText: '#15803d', icon: 'forkKnife' },
  snack:     { label: 'Lanche',        accent: '#2563eb', light: '#bfdbfe', accentText: '#1d4ed8', icon: 'leaf' },
  dinner:    { label: 'Jantar',        accent: '#9333ea', light: '#e9d5ff', accentText: '#7e22ce', icon: 'moon' },
  supper:    { label: 'Ceia',          accent: '#475569', light: '#cbd5e1', accentText: '#334155', icon: 'cookie' },
};

const FALLBACK = { label: 'Refeição', accent: '#475569', light: '#cbd5e1', accentText: '#334155', icon: 'bowlFood' as keyof typeof ICON_PATHS };
