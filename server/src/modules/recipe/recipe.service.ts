...
import { FoodItemPublic, RecipePublic, RecipeTotals, foodItemSelect, recipeSelect } from './types';
...
export class RecipeService {
  ...
  async findAllByPatient(callerId: string, patientId: string): Promise<RecipePublic[]> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const recipes = await this.prisma.recipe.findMany({ where: { patientId }, select: recipeSelect, orderBy: { createdAt: 'desc' } });
      return recipes.map(r => ({ ...r, totals: this.computeTotals(r.items) }));
    } catch (error) {
      mapPrismaError(error, 'Erro ao buscar receitas');
    }
  }

  async findOne(callerId: string, patientId: string, id: string): Promise<RecipePublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const recipe = await this.prisma.recipe.findFirst({ where: { id, patientId }, select: recipeSelect });
  ...
  async update(callerId: string, patientId: string, id: string, dto: UpdateRecipeDto): Promise<RecipePublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const recipe = await this.prisma.recipe.update({ where: { id, patientId }, data: { ...(dto.name !== undefined && { name: dto.name }) }, select: recipeSelect });
      return { ...recipe, totals: this.computeTotals(recipe.items) };
    } catch (error) {
      mapPrismaError(error, 'Erro ao atualizar receita', { p2025: 'Receita não encontrada' });
    }
  }
  ...
  async remove(callerId: string, patientId: string, id: string): Promise<{ id: string }> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      return await this.prisma.recipe.delete({ where: { id, patientId }, select: { id: true } });
    } catch (error) {
      mapPrismaError(error, 'Erro ao deletar receita', { p2025: 'Receita não encontrada' });
    }
  }

  async addFoodItem(callerId: string, patientId: string, recipeId: string, dto: AddFoodItemDto): Promise<FoodItemPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
  ...
  async addPrivateFoodItem(callerId: string, patientId: string, recipeId: string, dto: AddRecipePrivateFoodItemDto): Promise<FoodItemPublic> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
  ...
  async removeItem(callerId: string, patientId: string, recipeId: string, itemId: string): Promise<{ id: string }> {
    await this.patientAccess.resolve(callerId, patientId);
    try {
      const result = await this.prisma.foodItem.deleteMany({ where: { id: itemId, recipeId, recipe: { patientId } } });
      if (result.count === 0) throw new NotFoundException('Item não encontrado na receita');
      return { id: itemId };
    } catch (error) {
      mapPrismaError(error, 'Erro ao remover item da receita', { p2025: 'Item não encontrado na receita' });
    }
  }
}
