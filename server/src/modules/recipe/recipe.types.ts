...
export type RecipePublic = Pick<Recipe, 'id' | 'name' | 'calories' | 'protein' | 'carbs' | 'fat' | 'userId' | 'createdAt' | 'updatedAt'> & {
  items: FoodItemPublic[];
  totals: RecipeTotals;
};

export type RecipeSummary = Pick<Recipe, 'id' | 'name' | 'calories' | 'protein' | 'carbs' | 'fat' | 'userId'> & {
  totals: RecipeTotals;
};
