...
export async function loadLivsmedelsverket(maxItems: number = 99999): Promise<FoodEntry[]> {
  try {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      if (!food?.namn || !isValidFoodNameLoose(food.namn)) continue
      ...
      try {
        ...
        const translatedName = translateName(food.namn)

        entries.push({
          name: translatedName,
          nameOriginal: food.namn !== translatedName ? food.namn : null,
          source: 'LIVS',
