...
export async function loadOpenFoodFacts(): Promise<FoodEntry[]> {
  ...
  // Fase 2: busca por categorias genéricas BR (alimentos não-marca)
  const BR_CATEGORIES = [
    'en:fruits', 'en:vegetables', 'en:cereals-and-their-products',
    'en:dairy-products', 'en:meats', 'en:fish-and-seafood',
    'en:legumes', 'en:nuts', 'en:eggs', 'en:breads',
    'en:rices', 'en:pastas', 'en:oils-and-fats',
    'en:soups', 'en:sauces-and-condiments', 'en:snacks',
    'en:beverages', 'en:frozen-foods', 'en:canned-foods',
  ]

  console.log(`\n[OFF] Fase 2: buscando ${BR_CATEGORIES.length} categorias genéricas BR...`)
  let catIdx = 0
  for (const cat of BR_CATEGORIES) {
    ...
    for (let page = 1; page <= 4; page++) {
        ...
        try {
          const r = await axios.get('https://world.openfoodfacts.org/api/v2/search', {
            params: {
              countries_tags: 'en:brazil',
              categories_tags: cat,
              fields: 'product_name,product_name_pt,nutriments,unique_scans_n,code,categories_tags',
              sort_by: 'unique_scans_n',
              page_size: PAGE_SIZE,
              page,
            },
      ...
      for (const p of products) {
        ...
        foods.push({
          name, nameOriginal: name, source: 'OFF', sourceId: p.code,
          category: Array.isArray(p.categories_tags) ? p.categories_tags[0] ?? null : null,
          priority: 40, calories, protein, carbs, fat,
          fiber: fiber > 0 ? fiber : null,
          sodium: sodium > 0 ? sodium : null,
        })
        catCount++
    ...
    console.log(`[OFF] cat ${catIdx}/${BR_CATEGORIES.length} — ${cat.replace('en:','')}: +${catCount} (total ${foods.length})`)
    await sleep(REQUEST_DELAY_MS)
  }
